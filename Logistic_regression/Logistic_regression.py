...
def Logistic_regression(Xtrain, ytrain, a=0.001, iters=50):
	"""
	:param Xtrain: 训练样本特征
	:param ytrain: 训练样本标签
	:param a: 学习率
	...
	while i < iters:
		for j in range(tot_num):  # 每个样本的特征
			...
			w += a * (x * y - (expwx * x) / (1 + expwx))  # 等式后面为逻辑回归的对数似然函数，需要求其极大值，
...
def score(w, Xtest, ytest):
	...
	p = np.exp(Xtest.dot(w)) / (1 + np.exp(Xtest.dot(w)))
	print(p)
...
if __name__ == '__main__':
	...
	w = Logistic_regression(Xtrain, ytrain, a=0.0001, iters=1000000)
	score = score(w, Xtest, ytest)
	time1 = time()
